"use client";

import { useState, FormEvent } from "react";
import CTASection from "@/components/CTASection";
import AppScreenshots from "@/components/AppScreenshots";

export default function ContactContent() {
  const [formData, setFormData] = useState({
    naam: "",
    email: "",
    bericht: "",
  });
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setSubmitted(true);
  };

  return (
    <>
      {/* Hero */}
      <section
        className="py-16 lg:py-24"
        style={{
          background: "linear-gradient(to bottom, #f0f2f8, #ffffff)",
        }}
      >
        <div className="mx-auto max-w-7xl px-6 text-center">
          <h1 className="font-heading text-4xl font-bold text-[#211f54] md:text-5xl">
            Contact
          </h1>
        </div>
      </section>

      {/* Contact Details */}
      <section className="py-16 lg:py-24">
        <div className="mx-auto max-w-5xl px-6">
          <div className="grid gap-8 md:grid-cols-3">
            {/* Email */}
            <div className="rounded-2xl bg-[#f7f9ff] p-8 text-center shadow-sm">
              <div className="mb-4 flex justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-10 w-10 text-[#0e3065]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25H4.5a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5H4.5a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75"
                  />
                </svg>
              </div>
              <p className="mb-1 text-sm font-semibold uppercase tracking-wide text-[#4a5568]">
                E-mail
              </p>
              <a
                href="mailto:direct@vaneck.com"
                className="text-lg font-semibold text-[#0e3065] transition-colors hover:underline"
              >
                direct@vaneck.com
              </a>
            </div>

            {/* Phone */}
            <div className="rounded-2xl bg-[#f7f9ff] p-8 text-center shadow-sm">
              <div className="mb-4 flex justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-10 w-10 text-[#0e3065]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"
                  />
                </svg>
              </div>
              <p className="mb-1 text-sm font-semibold uppercase tracking-wide text-[#4a5568]">
                Telefoon
              </p>
              <a
                href="tel:+31208085543"
                className="text-lg font-semibold text-[#0e3065] transition-colors hover:underline"
              >
                +31 (0)20 808 5543
              </a>
            </div>

            {/* Address */}
            <div className="rounded-2xl bg-[#f7f9ff] p-8 text-center shadow-sm">
              <div className="mb-4 flex justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-10 w-10 text-[#0e3065]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"
                  />
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"
                  />
                </svg>
              </div>
              <p className="mb-1 text-sm font-semibold uppercase tracking-wide text-[#4a5568]">
                Adres
              </p>
              <p className="text-lg font-semibold text-[#0e3065]">
                Barbara Strozzilaan 310,
                <br />
                1083 HN Amsterdam
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Complaint Form */}
      <section className="bg-[#f2f3f7] py-16 lg:py-24">
        <div className="mx-auto max-w-2xl px-6">
          <div className="mb-10 text-center">
            <h2 className="font-heading text-3xl font-bold text-[#211f54] md:text-4xl">
              Klachtenformulier
            </h2>
            <p className="mt-4 text-base leading-relaxed text-[#4a5568]">
              Heeft u een klacht? Laat het ons weten via onderstaand formulier.
            </p>
          </div>

          {submitted ? (
            <div className="rounded-xl border border-green-200 bg-green-50 px-6 py-8 text-center">
              <p className="text-lg font-semibold text-green-800">
                Bedankt voor uw bericht. We nemen zo snel mogelijk contact met u
                op.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label
                  htmlFor="naam"
                  className="mb-2 block font-semibold text-[#211f54]"
                >
                  Naam
                </label>
                <input
                  type="text"
                  id="naam"
                  name="naam"
                  required
                  value={formData.naam}
                  onChange={(e) =>
                    setFormData({ ...formData, naam: e.target.value })
                  }
                  className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-[#211f54] outline-none transition-colors focus:border-[#0e3065] focus:ring-1 focus:ring-[#0e3065]"
                />
              </div>

              <div>
                <label
                  htmlFor="email"
                  className="mb-2 block font-semibold text-[#211f54]"
                >
                  E-mailadres
                </label>
                <input
                  type="email"
                  id="email"
                  name="email"
                  required
                  value={formData.email}
                  onChange={(e) =>
                    setFormData({ ...formData, email: e.target.value })
                  }
                  className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-[#211f54] outline-none transition-colors focus:border-[#0e3065] focus:ring-1 focus:ring-[#0e3065]"
                />
              </div>

              <div>
                <label
                  htmlFor="bericht"
                  className="mb-2 block font-semibold text-[#211f54]"
                >
                  Uw bericht
                </label>
                <textarea
                  id="bericht"
                  name="bericht"
                  required
                  rows={6}
                  value={formData.bericht}
                  onChange={(e) =>
                    setFormData({ ...formData, bericht: e.target.value })
                  }
                  className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-[#211f54] outline-none transition-colors focus:border-[#0e3065] focus:ring-1 focus:ring-[#0e3065]"
                />
              </div>

              <div className="text-center">
                <button
                  type="submit"
                  className="rounded-full bg-[#0e3065] px-8 py-3 text-base font-semibold text-white transition-colors hover:bg-[#0a2450]"
                >
                  Verstuur
                </button>
              </div>
            </form>
          )}
        </div>
      </section>

      {/* CTA Section */}
      <CTASection />

      {/* App Screenshots */}
      <AppScreenshots />
    </>
  );
}
